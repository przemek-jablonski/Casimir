...
private extension LoremIpsum {
    static var full: String { lorem }
    static var words: [String] { LoremIpsum.full.components(separatedBy: " ") }
    static var sentences: [String] { LoremIpsum.full.components(separatedBy: ". ") }
}
...
public extension LoremIpsum {
    static var random: String {
        if Bool.random() {
            return (1...3)
                .map({ _ in
                    LoremIpsum.words.random.filter({ CharacterSet.letters.contains($0.unicodeScalars.first!) })
                })
                .joined(separator: ", ")
                .uppercasedFirst
        } else {
